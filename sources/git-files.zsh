...
function zaw-src-git-files-raw() {
    ...
    actions=(zaw-src-git-status-add zaw-src-git-status-add-p zaw-src-git-status-reset zaw-src-git-status-checkout zaw-callback-edit-file zaw-src-git-status-rm zaw-callback-append-to-buffer)
    act_descriptions=("add" "add -p" "reset" "checkout" "edit" "rm" "append to edit buffer")
    options=(-m -n)
    return 0
}
